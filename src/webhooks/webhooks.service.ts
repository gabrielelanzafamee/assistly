...
export class WebhooksService {
	...
	private async getAssistantAndPhone(
		phoneId: string,
	...
	private async handleInactiveAssistant(
		conversation: any,
		from: string,
		body: string,
		smsStatus: string,
	...
	async callbackSms(params: ITwilioSmsCallback, phoneId: string) {
		...
		const { assistant, phone } = await this.getAssistantAndPhone(
			phoneId,
			AssistantType.SMS,
		);

		const conversation =
			await this.conversationsService.getConversationOrCreate({
		...
		if (!assistant.isActive) {
			await this.handleInactiveAssistant(
				conversation,
				from,
				body,
				smsStatus,
		...
		await this.usageService.recordUsage(phone.organization._id.toString(), 'twilio', 'whatsapp', 1);

		if (!conversation.automaticReply) return true;

		const history = await this.messagesService.listByConversation(
			conversation._id.toString(),
			phone.organization._id.toString(),
		);
	...
	async callbackWhatsapp(params: ITwilioWhatsappCallback, phoneId: string) {
		const { phone, assistant } = await this.getAssistantAndPhone(
			phoneId,
	...
	async callbackCall(params: ITwilioVoiceCallback, phoneId: string) {
		...
		const calls = await this.callsService.getCallsByStatus(
			params.From,
			phone.organization._id.toString(),
	...
	async callbackStatus(params: ITwilioVoiceStatusCallback, phoneId: string) {
		...
		const { phone } = await this.getAssistantAndPhone(
			phoneId,
			AssistantType.SMS,
		);
		const call = await this.callsService.getCallTwilio(
			From,
			phone._id.toString(),
			phone.organization._id.toString(),
		);
		...
		// update status
		await this.callsService.update(
			call._id.toString(),
